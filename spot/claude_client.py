...
class AggressiveClaudeClient:
    ...
    async def aggressive_analyze(self, market_data):
        ...
        if instant_decision:
            logging.info(f"⚡ 즉시결정: {instant_decision['action']} - {instant_decision['reason']}")
    ...
    def _get_aggressive_cache(self, data):
        ...
        if cached and self._is_aggressive_cache_fresh(cached):
            return {
                ...
                "confidence": cached.get('confidence', 0.6) * 0.9  # 캐시는 약간 낮은 신뢰도
            }
    ...
    def _needs_aggressive_ai(self, data):
        ...
        urgent_cases = [
            ...
            # 🔥 정기 점검 더 자주
            not self.last_ai_call or 
            (datetime.now() - self.last_ai_call > timedelta(minutes=30)),  # 1시간 → 30분
...
즉시 JSON만 응답:"""
        ...
        for attempt in range(3):
            try:
                ...
                if result:
                    ...
                    logging.info(f"🤖 공격적 AI 분석 완료 (호출#{self.call_count}, 시도{attempt+1})")
                    return result
    ...
    def _parse_aggressive_response(self, content):
        ...
        try:
            ...
            if '{' in content:
                ...
                if action in ['buy', 'sell', 'hold']:
                    return {
                        "action": action,
                        "reason": json_data.get('reason', 'ai_분석')[:50],  # 더 긴 사유
                        "confidence": min(json_data.get('confidence', 0.7) * 1.1, 0.95)  # 공격적 신뢰도 증가
                    }
        ...
        elif any(word in content_lower for word in ['급등', '매도', 'sell', '익절', '손절']):
    ...
    def get_performance_stats(self):
        ...
        # 매도 결정만 추출
        sell_decisions = [d for d in self.decisions if d.get('action') == 'sell']
    ...
    def get_optimization_stats(self):
        ...
        savings_rate = (decisions_without_ai / max(len(self.decisions), 1)) * 100
        ...
        return {
            ...
            'cache_size': len(self.decision_cache),
            'emergency_mode': self.emergency_mode,
            'estimated_daily_cost': f"${self.call_count * 0.003:.3f}"  # 공격적 모드는 비용 약간 상승
        }